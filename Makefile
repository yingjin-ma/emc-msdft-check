...
F90srcs = MSDFT_MAIN.f90 MSDFT_RDINCAR.f90 MSDFT_COMB.f90 MSDFT_MCCONF.f90 MSDFT_BLWCAS.f90 MSDFT_CHECKFILE.f90 MSDFT_MCFILE.f90\
	  ...
	  Derivaties_deltaR.f90 Transform_Nindex.f90 Solver_gmres.f90 mgmres.f90 Solver_Augmented_Hessian.f90 Davidson.f90\
...
#mac OS with intel mkls
#FLIB = -L${MKLROOT}/lib -Wl,-rpath,${MKLROOT}/lib -lmkl_intel_ilp64 -lmkl_sequential -lmkl_core -lpthread -lm -ldl

$(Program_name):$(F90objs) $(F77objs) $(CPPobjs)
	$(Complier2) $(Link) $(Program_name) $(F90objs) $(F77objs) $(CPPobjs) $(LIB) $(FLIB)


.SUFFIXES : .cpp .F .f90 .o 

.cpp.o:
	g++ -O2 -g -c $< -m64 -I"${MKLROOT}/include"

.f90.o:
	$(Complier2) -O2 -g -c -fallow-argument-mismatch -Wno-argument-mismatch $<  -m64 -I"${MKLROOT}/include"
