...
#include <sys/time.h>
#include <sys/stat.h>
#define THREADS_PER_BLOCK_AG 192
...
__global__ void computeA_22(int *total,int *occ,int noffset,int noffset1,
    ...
    if(n1<total[n]&&m1<occ[m]){
        // for(int m1=0;m1<occ[m];m1++){
            for(int j1=0;j1<occ[j];j1++){
...
__global__ void computeG_11(int *total,int *occ,int noffset,
                           int moffset,int moffset1,int joffset,int joffset1,int koffset,
                           int n,int m,int j,int k,int norb,int nocc,
                           double *G,double *T,double *D,int stride2norb,int stride3norb)
{
    
    int idx= blockIdx.x * blockDim.x + threadIdx.x;
    int n1 = idx % total[n];
    int m1= (idx / total[n]) % occ[m];
    int j1 = (idx / (total[n] * occ[m])) % occ[j];
    int k1 = idx / (total[n] * occ[m] * occ[j]);
    if(n1<total[n]&&m1<occ[m]&&j1<occ[j]&&k1<total[k]){
        int mr=m1+moffset1;
        int jr=j1+joffset1;
        int ni=n1+noffset;
        int mi=m1+moffset;
...
__global__ void computeG_22(int *total,int *occ,int noffset,
                           int moffset,int moffset1,int joffset,int joffset1,int koffset,int koffset1,int loffset,int loffset1,
                           int kooffset,int kooffset1,
                           int n,int m,int j,int ko,int k,int l,int norb,int nocc,
                           double *G,double *p,double *U,int stride2norb,int stride3norb,
                           int stride2nocc,int stride3nocc){

    int idx= blockIdx.x * blockDim.x + threadIdx.x;
    int n1 = idx % total[n];
    int m1= (idx / total[n]) % occ[m];
    int ko1 = (idx / (total[n] * occ[m])) % occ[ko];
    int j1 = idx / (total[n] * occ[m] * occ[ko]); 
    if(n1<total[n]&&m1<occ[m]&&j1<occ[j]&&ko1<total[ko]){
        for(int k1=0;k1<occ[k];k1++){
            for(int l1=0;l1<occ[l];l1++){
                int mr=m1+moffset1;
                int jr=j1+joffset1;
...
extern "C" void qaz_(int *nsub,double* A,double *T,double *D,
                          int *norbA,int *noccD,
    ...
    for(int n=0;n<orbnsub;n++){
        ...
        for(int m=0;m<orbnsub;m++){
            ...
            for(int j=0;j<orbnsub;j++){
                ...
                computeA_11<<<block_num_AG11,THREADS_PER_BLOCK_AG,0,stream1>>>(total_d,occ_d,noffset,noffset1,moffset,moffset1,joffset,joffset1,n,m,j,norb,nocc,A_d,T_d,D_d);
    ...
    // cudaFree(total_d);
    // cudaFree(A_d);
    // CUDA_CHECK_CALL(cudaFree(T_d));
    // CUDA_CHECK_CALL(cudaFree(D_d));
    double *p_d,*U_d;
            ...
            for(int j=0;j<orbnsub;j++){
                ...
                for(int k=0;k<orbnsub;k++){
                    ...
                    for(int l=0;l<orbnsub;l++){
                        ...
                        computeA_22<<<block_num_AG22,THREADS_PER_BLOCK_AG,0,stream1>>>(total_d,occ_d,noffset,noffset1,
    ...
    CUDA_CHECK_CALL(cudaMemcpy( A,A_d,sizeof(double)*stride2norb,cudaMemcpyDeviceToHost));
    // CUDA_CHECK_CALL(cudaFree(occ_d));
    // CUDA_CHECK_CALL(cudaFree(total_d));
    CUDA_CHECK_CALL(cudaFree(A_d));
    // CUDA_CHECK_CALL(cudaFree(p_d));
    // CUDA_CHECK_CALL(cudaFree(U_d));
    ...
    // gettimeofday(&start,NULL);
    double *G_d;
    CUDA_CHECK_CALL(cudaMalloc( (void **)&G_d,sizeof(double)*stride3norb*norb));
    CUDA_CHECK_CALL(cudaMemcpy(G_d,G,sizeof(double)*stride3norb*norb,cudaMemcpyHostToDevice));
    ...
    for(int n=0;n<orbnsub;n++){
        ...
        for(int m=0;m<orbnsub;m++){
            ...
            for(int j=0;j<orbnsub;j++){
                ...
                koffset1=0;
                for(int k=0;k<orbnsub;k++){
                    int block_num_AG22=(orb_total[n]*orb_occ[m]*orb_occ[j]*orb_total[k]+THREADS_PER_BLOCK_AG-1)/THREADS_PER_BLOCK_AG;
                    computeG_11<<<block_num_AG22,THREADS_PER_BLOCK_AG,0,stream2>>>(total_d,occ_d,noffset,
                        moffset,moffset1,joffset,joffset1,koffset,
                        n,m,j,k,norb,nocc,
                        G_d,T_d,D_d,stride2norb,stride3norb);
                    cudaDeviceSynchronize();
    ...
    // CUDA_CHECK_CALL(cudaMemcpy( G,G_d,sizeof(double)*stride3norb*norb,cudaMemcpyDeviceToHost));
    // CUDA_CHECK_CALL(cudaFree(G_d));
    CUDA_CHECK_CALL(cudaFree(T_d));
    CUDA_CHECK_CALL(cudaFree(D_d));
    // CUDA_CHECK_CALL(cudaFree(total_d));
    // CUDA_CHECK_CALL(cudaFree(occ_d));
                ...
                for(int j=0;j<orbnsub;j++){
                    ...
                    for(int k=0;k<orbnsub;k++){
                        ...
                        for(int l=0;l<orbnsub;l++){
                            ...
                            int block_num_AG22=(orb_total[n]*orb_occ[m]*orb_occ[j]*orb_total[ko]+THREADS_PER_BLOCK_AG-1)/THREADS_PER_BLOCK_AG;
                            computeG_22<<<block_num_AG22,THREADS_PER_BLOCK_AG,0,stream2>>>(total_d,occ_d,noffset,
                            moffset,moffset1,joffset,joffset1,koffset,koffset1,loffset,loffset1,
                            kooffset,kooffset1,n,m,j,ko,k,l,norb,nocc,
                            G_d,p_d,U_d,stride2norb,stride3norb,stride2nocc,stride3nocc);
                            cudaDeviceSynchronize();
