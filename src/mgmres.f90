! interface
!     subroutine serial_host(n, ia, ja, a, x, w) bind(C, name="serial_host")
!         use iso_c_binding
!         implicit none
...
subroutine atx_cr ( n, nz_num, ia, ja, a, x, w )
  ...
  subroutine ax_crDouble ( n, nz_num, ia, ja, a, x, w )
    implicit none
  ...
  subroutine pmgmres_crd ( n, nz_num, ia, ja, a, x, rhs, itr_max, mr,tol_abs, tol_rel)
  ...
  ! real ( kind = rk ) l(ia(n+1)+1)
    ...
    do itr = 1, itr_max
      ...
      ! call entrywise_csr(n,nz_num,ia,ja,a,x,r,SHPnonzeros,SHProwoffset,SHPcolindex,SHPvalues,sx)
      ! call ax_cr ( n, SHPnonzeros, SHProwoffset, SHPcolindex, SHPvalues, sx, rs )
      call xjf_csr(n,nz_num,ia,ja,a,x,r)
      ! r=rs+r
      write(*,*) r(:100)
      ...
      do k = 1, mr
        ...
        ! sx=v(1:n,k)
        ! rs=0.0
        ! call entrywise_csr(n,nz_num,ia,ja,a,v(1:n,k),v(1:n,k+1),SHPnonzeros,SHProwoffset,SHPcolindex,SHPvalues,sx)
        ! call ax_cr(n,SHPnonzeros,SHProwoffset,SHPcolindex,SHPvalues,sx,rs)
        call xjf_csr(n,nz_num,ia,ja,a,v(1:n,k), v(1:n,k+1))
        ! v(1:n,k+1)=v(1:n,k+1)+rs
        ! if(k==1)then
        !   write(*,*) n
        !   write(*,*) v(1:n,k+1)
        !   write(*,*) "897987"
  ...
  subroutine matrix_to_csr(spmat,rows,cols)
    use matrix
    integer::rows,cols,size_rowoffset
    double precision::spmat(rows,cols)
    integer:: i, j, k, l
    nonzeros=0
    ...
    write(*,*)"spmat_nonzeros: ",nonzeros
    size_rowoffset=rows+1
    allocate(rowoffset(size_rowoffset))
    allocate(colindex(nonzeros))
    allocate(values(nonzeros))
    ...
    do i = 1, rows
          do j = 1, cols
              if (spmat(i, j) /= 0) then
                  nonzeros = nonzeros + 1
                  values(nonzeros)=spmat(i,j)
                  colindex(nonzeros)=j
              end if
          end do
          rowoffset(i+1)=nonzeros+1
    end do
  
  end subroutine matrix_to_csr

  subroutine Smatrix_to_csr(spmat,rows,cols)
    use matrix
    integer::rows,cols,size_rowoffset
    real(4)::spmat(rows,cols)
    integer:: i, j, k, l
    SHPnonzeros=0
    ...
    do i = 1, rows
          do j = 1, cols
              if (spmat(i, j) /= 0) then
                SHPnonzeros = SHPnonzeros + 1
              end if
          end do
    end do
    write(*,*)"spmat_nonzeros: ",SHPnonzeros
    size_rowoffset=rows+1
    allocate(SHProwoffset(size_rowoffset))
    allocate(SHPcolindex(SHPnonzeros))
    allocate(SHPvalues(SHPnonzeros))
    ! write(*,*) spmat(:,1)
    ...
    do i = 1, rows
          do j = 1, cols
              if (spmat(i, j) /= 0) then
                  SHPnonzeros = SHPnonzeros + 1
                  SHPvalues(SHPnonzeros)=spmat(i,j)
                  SHPcolindex(SHPnonzeros)=j
                  ! write(*,*)SHPvalues(SHPnonzeros)
                  ! write(*,*) SHPcolindex(SHPnonzeros)
              end if
          end do
          SHProwoffset(i+1)=SHPnonzeros+1
    end do
  
  end subroutine Smatrix_to_csr
  
  subroutine matrix_to_coo(spmat,rows,cols)
    use matrix
    integer::rows,cols
    double precision::spmat(rows,cols)
    integer:: i, j, k, l
    ...
    write(*,*)
    write(*,*)"spmat_nonzeros: ",nonzeros
    allocate(coo_cols(nonzeros))
    allocate(coo_rows(nonzeros))
    allocate(coo_values(nonzeros))
    nonzeros=0
    do i = 1, rows
          do j = 1, cols
              if (spmat(i, j) /= 0) then
                  nonzeros = nonzeros + 1
                  coo_values(nonzeros)=spmat(i,j)
                  coo_rows(nonzeros)=i
                  coo_cols(nonzeros)=j
              end if
          end do
    ...
    write(*,*)"spmat_nonzeros: ",nonzeros
  end subroutine matrix_to_coo

  subroutine pmgmres_cr_d ( n, nz_num, ia, ja, a, x, rhs, itr_max, mr,tol_abs, tol_rel)
  ...
  subroutine pmgmres_crs ( n, nz_num, ia, ja, a, x, rhs, itr_max, mr,tol_abs, tol_rel)
