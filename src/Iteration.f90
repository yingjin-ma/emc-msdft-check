      Subroutine Iteration(icycle,pre_solved_internal)
        ...
        write(6,*)"++++ Entering Iteration.f90 ++++ "
        call flush(6) 
        ...
        write(6,*)"++++ Iteration.f90 ++++ "
        call flush(6) 

        !       Sum|delta_R| threshold 
        if(icycle.eq.1)then
          iter_threshold=0.01d0 !testing yma
        ...
        do i=1,orb%nsub
              ...
              if(j.ne.k)then 
                if(j.gt.orb%occ(i).and.k.gt.orb%occ(i))then
                  ...
                  if(icycle.gt.0)then
                    ...
                    if(dabs(mat3(i)%A_Ap(j,k)).lt.1.0e-9)then
          ...
          !          call print_mat(orb%total(i),orb%total(i),mat3(i)%A_Ap)
          !          call print_mat(orb%total(i),orb%total(i),mat3(i)%Rn)
            !mat3(i)%Rn=0.0d0 
        end do
        ...
        do 
          ...
          if(method(icycle).eq."microit")then
            ! E'=UB-BU 
            !write(*,*)"=======",method 
            call derivaties(iloop)
          else if(method(icycle).eq."occhess")then
            call derivaties(iloop)
          else if(method(icycle).eq."direct2")then 
            call derivaties2(icycle,iloop)
          else if(method(icycle).eq."direct3")then
            call derivaties3(icycle,iloop)
          else if(method(icycle).eq."newraph")then
            call derivaties4(icycle,iloop)
          else if(method(icycle).eq."cp-hess")then
            call derivaties(iloop)
          else
            write(*,*)"This should not be a micro-iterative method" 
            stop 
          end if
          do i=1,orb%nsub
              ...
              do j=1,orb%total(i)
                do k=1,orb%total(i)         
                  if(j.ne.k)then 
                    ...
                    else if(dabs(mat3(i)%A_Ap(j,k)).lt.1.0e-9)then !only this should work
                    else
          ...
          do i=1,orb%nsub
            do while(mat3(i)%Rsub.gt.1.0d0)
              mat3(i)%Rsub=mat3(i)%Rsub/1.25d0
              mat3(i)%deltR=mat3(i)%deltR/1.25d0
            end do
          end do
...
2436    format(A3,A7)
        ...
        write(6,*)"Done the iteration.f90"
        call flush(6)  
