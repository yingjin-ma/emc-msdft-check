...
9999    continue
      ...
      End Subroutine Solver_WMK


! Assuming lamda eq 1
      Subroutine residual_WMK(ndim,D_lamda,UM,BM,AM,dR,RM)

        integer::ndim
        double precision::D_lamda 
        double precision::UM(ndim,ndim)       
        double precision::BM(ndim,ndim)       
        double precision::AM(ndim,ndim)       
        double precision::dR(ndim,ndim)       
        double precision::RM(ndim,ndim)      
       
        double precision D_epsilon  

        double precision AA(ndim,ndim)
        double precision UB(ndim,ndim),BU(ndim,ndim)
        double precision AdR(ndim,ndim),dRA(ndim,ndim)

        double precision TM(ndim,ndim),TM1(ndim,ndim)

        RM=0.0d0
 
        print *,"D_lamda",D_lamda

        call MdXM(ndim,UM,BM,UB)
        call MdXM(ndim,BM,UM,BU)

!        call print_mat(ndim,ndim,BM,6)

        do i=1,ndim
          do j=1,ndim
            AA(i,j)=AM(i,j)+AM(j,i) ! A + A^+
            TM(i,j)=AM(i,j)-AM(j,i) ! A - A^+
          end do
        end do
        print *," A+A^+"
        call print_mat(ndim,ndim,AA,6)

        !print *," dR"
        !call print_mat(ndim,ndim, dR,6)

        call MXM(ndim,AA,dR,AdR)
        call MXM(ndim,dR,AA,dRA)

        !print *,"AdR"
        !call print_mat(ndim,ndim,AdR,6)
        !print *,"dRA"
        !call print_mat(ndim,ndim,dRA,6)
        print *,"-1/2(ARd+dRA)"
        call print_mat(ndim,ndim,-0.5d0*(AdR+dRA),6)

! level shift  (TM or AM) ?  both OK!
        !call MdXM(ndim,AM,dR,TM1) 
        call MdXM(ndim,TM,dR,TM1) 
        call Trace(ndim,TM1,D_epsilon)
        D_epsilon=D_epsilon*D_lamda 
        print *,"D_epsilon",D_epsilon         
        print *,"UdB-dBU"
        call print_mat(ndim,ndim,UB-BU,6)

        print *,"lamda*epsilon*dR"
        call print_mat(ndim,ndim,D_lamda*D_epsilon*dR,6)

        RM=UB-BU-0.5d0*(AdR+dRA)-D_lamda*D_epsilon*dR 

        print *," RM"
        call print_mat(ndim,ndim,RM,6)

        print *,"A-A+"
        call print_mat(ndim,ndim,TM,6)
!        stop

      end Subroutine residual_WMK 


      Subroutine residual_WMK2(ndim,nval,valid,D_lamda,UM,BM,AM,dR,RM)

        integer::ndim,nval
        integer::valid(nval,2)
        double precision::D_lamda 
        double precision::UM(ndim,ndim)       
        double precision::BM(ndim,ndim)       
        double precision::AM(ndim,ndim)       
        ...
        print *,"D_lamda",D_lamda
        print *,"valid-1",valid(:,1)
        print *,"valid-2",valid(:,2)
